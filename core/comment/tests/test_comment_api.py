...
def get_comment(get_profile, get_post):
    ...
    comment = Comments.objects.create(
        user=profile, text="Test Comment.", post=post, star=3
    )
    return comment
...
class TestCommentApi:
    ...
    def test_create_comment_response_201(
        self, api_client, get_user, get_post, get_comment
    ):
        user = get_user
        api_client.force_login(user=user)
        url = reverse("comment-api-v1:Comments-list")
        ...
        assert get_comment.__class__.objects.filter(
            text=data["text"], post=data["post"]
        ).exists()
